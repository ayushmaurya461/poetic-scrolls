    ...
    >
      <div className="container mx-auto py-8">
        ...
        <div className="relative">
          {/* Timeline line */}
          <div className="hidden md:block absolute md:left-1/2 h-full w-[2px] bg-gray-400/50 transform -translate-x-1/2" />
          ...
          <div className="space-y-12">
            {experiences.map((exp, index) => (
              <div key={exp.company} className="relative">
                ...
                >
                  {/* Timeline dot */}
                  <div className="hidden md:block absolute md:left-1/2 w-4 h-4 bg-dark rounded-full transform -translate-x-1/2 mt-6" />
                  
                  {/* Content */}
                  <div className={`w-full md:w-1/2 ${index % 2 === 0 ? 'md:pl-8' : 'md:pr-8'}`}>
                    <div 
                      className="glass p-6 rounded-xl cursor-pointer border-2 border-gray-400/50 hover:bg-white/5 hover:border-gray-300/70 transition-all"
                      onClick={() => setSelectedExp(exp)}
                    >
                      <div className="mb-4">
                        <h3 className="text-xl font-display font-bold text-gray-800">{exp.company}</h3>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {exp.technologies.map((tech) => (
                          <span key={tech} className="text-xs px-2 py-1 rounded-full bg-gray-200 text-gray-800">
                            {tech}
                          </span>
                        ))}
                      </div>
      ...
      >
        {selectedExp && (
          <div className="p-6">
            <h2 className="text-2xl font-display font-bold text-gray-800 mb-2">{selectedExp.company}</h2>
            <p className="text-gray-600 mb-6">{selectedExp.role} • {selectedExp.period}</p>
            
            <div className="space-y-6">
              <div>
                <h3 className="text-lg font-semibold text-gray-800 mb-2">Key Achievements</h3>
